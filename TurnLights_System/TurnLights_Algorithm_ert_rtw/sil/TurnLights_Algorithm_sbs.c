/*
 * TurnLights_Algorithm_sbs.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: TurnLights_Algorithm
 * Component Simulink Path: TurnLights_Algorithm
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                TurnLights_Algorithm_sbs
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("TurnLights_Model");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  ssSetInputPortSampleTime(S, 0, 0.5);
  ...
  /* using port based sample times */
  ssSetInputPortSampleTime(S, 1, 0.5);
  ...
  /* using port based sample times */
  ssSetInputPortSampleTime(S, 2, 0.5);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 0, 0.5);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 1, 0.5);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 2, 0.5);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 3, 0.5);
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 12, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
